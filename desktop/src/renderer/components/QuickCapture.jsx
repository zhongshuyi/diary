...
import { CheckCircle2, CloudSun, Code2, FileText, Film, Frown, ImagePlus, LoaderCircle, Meh, Music2, Smile, Sun, Type, UploadCloud, X } from 'lucide-react';
...
import { shouldResetQuickCaptureScroll, shouldShowEditorClose } from '../lib/quick-capture-presentation.mjs';

const moodOptions = [
  { value: '0.1', label: '阴天', icon: CloudSun },
  { value: '0.3', label: '低落', icon: Frown },
  { value: '0.5', label: '平常', icon: Meh },
  { value: '0.7', label: '平静', icon: Smile },
  { value: '0.9', label: '明亮', icon: Sun },
];
...
export function QuickCapture({ draft, setDraft, attachments, setAttachments, editing, inline = false, windowed = false, categoryOptions = defaultCategoryOptions, tagOptions = [], draftStatus = 'idle', onSave, onClose, onNotify }) {
  ...
  useEffect(() => {
    ...
    if (shouldResetQuickCaptureScroll({ windowed })) window.requestAnimationFrame(() => document.querySelector('.quick-capture-body')?.scrollTo({ top: 0 }));
  }, []);
