...
void main() {
  test('sorts prior same-day entries and excludes ineligible records', () {
    ...
    expect(reflection.onThisDayEntries.map((value) => value.id), [
      'recent',
      'older',
    ]);
    expect(
  ...
  test(
    'uses effective occurrence date and a content fallback for untitled records',
    () {
      final value = entry(
        'legacy',
  ...
  test(
    'breaks same-time on-this-day ties by ID and rotates a single candidate safely',
    () {
      final reflection = calculateTimelineReflection(
        entries: [
